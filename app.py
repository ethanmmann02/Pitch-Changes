...
# If you're using live Statcast pulls:
...
def _smooth_2d(H: np.ndarray, passes: int = 2) -> np.ndarray:
    """Light smoothing without extra deps."""
    if H.size == 0:
        return H
...
def location_heatmap_working(df_pt: pd.DataFrame, title: str, zmax=None):
    """2D histogram + smoothing + strike zone. Always works."""
...
def shares_by(df_recent: pd.DataFrame, df_base: pd.DataFrame, col: str) -> pd.DataFrame:
    if df_recent.empty and df_base.empty:
        return pd.DataFrame({col: [], "recent": [], "baseline": [], "diff_pp": []})
...
def window_split(df: pd.DataFrame, last_days: int, baseline_days: int):
    if df.empty or "game_date" not in df.columns or df["game_date"].dropna().empty:
        # Return empty windows safely
        return df.copy(), df.copy(), None, None, None
...
def postprocess(df: pd.DataFrame) -> pd.DataFrame:
    """Make sure required columns exist + compute derived fields."""
    ...
    # Batter handedness (only for heatmaps)
    ...
    # Team from home/away + inning_topbot
    ...
    # Arm angle
    if "arm_angle_deg" not in df.columns:
        if "arm_angle" in df.columns:
            df["arm_angle_deg"] = pd.to_numeric(df["arm_angle"], errors="coerce")
        elif {"release_pos_x", "release_pos_z"}.issubset(df.columns):
...
def pull_statcast_cached(start: str, end: str) -> pd.DataFrame:
    """Pull statcast and return raw df. Cached by date range."""
...
# Keep last-good dataset so the app never dies on a bad pull
if "df_data" not in st.session_state:
    st.session_state.df_data = pd.DataFrame()
if "last_ok_range" not in st.session_state:
    st.session_state.last_ok_range = None

with st.sidebar:
    st.header("Savant Pull (2025 Regular Season)")

    start_dt = st.date_input("Start date", value=date(2025, 3, 1))
    end_dt = st.date_input("End date", value=date(2025, 10, 31))
    reg_only = st.checkbox("Regular season only (game_type = R)", value=True)

    if st.button("Fetch / Refresh Statcast"):
        if end_dt < start_dt:
            st.error("End date must be after start date.")
        else:
            with st.spinner("Pulling Statcast from Baseball Savant..."):
                try:
                    df_raw = pull_statcast_cached(str(start_dt), str(end_dt))
                    df_new = postprocess(df_raw)

                    # Optional filter for regular season if the column exists
                    if reg_only and "game_type" in df_new.columns:
                        df_new = df_new[df_new["game_type"] == "R"].copy()

                    if df_new.empty:
                        st.warning("No rows returned for that date range. Keeping the previous dataset.")
                    else:
                        st.session_state.df_data = df_new
                        st.session_state.last_ok_range = (str(start_dt), str(end_dt), "R" if reg_only else "ALL")
                        st.success(f"Loaded {len(df_new):,} pitches.")
                except Exception as e:
                    st.warning(f"Statcast pull failed. Keeping the previous dataset. Error: {e}")
...
# If nothing loaded yet, don't crash — just instruct
if df.empty:
    st.info("No data loaded yet. Use the left sidebar and click **Fetch / Refresh Statcast**.")
    st.stop()

recent, baseline, recent_start, baseline_start, max_date = window_split(df, last_days, baseline_days)

cap = f"Loaded pitches: **{len(df):,}**"
if st.session_state.last_ok_range:
    cap += f" (last good pull: {st.session_state.last_ok_range})"
st.caption(cap)

if recent_start and baseline_start and max_date:
    st.caption(
...
if changes_view is None or changes_view.empty:
    st.info("Not enough data in these windows to compute changes. Try a bigger date range or lower min pitches.")
else:
    with col1:
...
# ✅ HARD GUARD: no pitchers, no crash
if pitchers.empty:
    st.warning("No pitchers found in the loaded dataset. Try a different date range and refresh.")
    st.stop()

pitcher_name = st.selectbox("Select pitcher", pitchers["player_name"].tolist())

sel = pitchers.loc[pitchers["player_name"] == pitcher_name, "pitcher"]
if sel.empty:
    st.warning("Pitcher selection produced no rows (unexpected). Try another pitcher or refresh.")
    st.stop()

pitcher_id = int(sel.iloc[0])
...
if not pitch_types:
    st.warning("No pitch types found for this pitcher in the loaded data.")
    st.stop()
...
if trend.empty:
    st.info("No trend data for this pitcher/pitch type in the loaded range.")
else:
